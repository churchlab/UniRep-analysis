...
def lev_dist_matrix(X): #takes in array of strings
    dist_matrix = np.zeros([X.shape[0],X.shape[0]])
    for i in range(X.shape[0]):
        for j in range(X.shape[0]):
            sim = Levenshtein.distance(X[i], X[j])
            dist_matrix[i,j] = sim
    return dist_matrix
    
def lev_sim_matrix(X): #takes in array of strings
    sim_matrix = np.zeros([X.shape[0],X.shape[0]])
    for i in range(X.shape[0]):
        for j in range(X.shape[0]):
            sim = Levenshtein.ratio(X[i], X[j])
            sim_matrix[i,j] = sim
    return sim_matrix # actually similarity matrix
